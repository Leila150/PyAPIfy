...
class PyAPIfy:
    def __init__(self,title='PyAPIfy API',version='0.1.0',debug=False):
        self.title=title; self.version=version; self.debug=debug; self.router=Router(); self._middleware=[]; self.errors={}; self._startup=[]; self._shutdown=[]; self.plugins=[]
    def route(self,path,methods=None,**opts):
        ...
        return deco
    def middleware(self,fn): self._middleware.append(fn); return fn
    ...
    async def _call(self,fn,request,params):
        ...
        for name,p in sig.parameters.items():
            ...
            if name in ('request','req') or ann is Request: kwargs[name]=request
                ...
                d=p.default.dependency; ds=inspect.signature(d); kw={'request':request} if 'request' in ds.parameters else {}; val=d(**kw); kwargs[name]=await val if inspect.isawaitable(val) else val
    ...
    async def _auth_async(self,auth,request):
        ...
        supplied=request.headers.get('Authorization') or request.headers.get('X-API-Key'); values=auth if isinstance(auth,(list,tuple,set)) else [auth]
        for v in values:
            if callable(v):
                r=v(request); r=await r if inspect.isawaitable(r) else r
                if r:return True
