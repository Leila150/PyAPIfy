        ...
        def decorate(fn):
            ...
            def wrapper(*args, **kwargs):
                try:
                    ...
                    self.logger.debug('Function cache bypassed for %s', fn.__name__)
                    return fn(*args, **kwargs)
                marker = object()
                value = self.get(identity, marker)
                if value is not marker:
                    self.logger.debug('Function cache hit: %s', prefix)
                    return value
                value = fn(*args, **kwargs)
                self.set(identity, value, ttl=ttl)
                self.logger.debug('Function cache populated: %s', prefix)
                return value
            return wrapper
        return decorate
...
class PyAPIfyRuntime:
    ...
    def __new__(cls, root=None, *, max_logs=5):
        resolved = Path(root).expanduser().resolve() if root else Path.cwd().resolve() / '.pyapify'
        key = str(resolved)
    ...
    def _rotate_logs(self):
        ...
        for index in range(self.max_logs - 1, 1, -1):
            src = self.logs_dir / f'latest_log_{index}.txt'
            dst = self.logs_dir / f'latest_log_{index + 1}.txt'
            if src.exists():
                src.replace(dst)
    ...
    def _create_logger(self):
        self._rotate_logs()
        logger = configure_logger(get_logger())
...
__all__ = ['PluginCache', 'PyAPIfyRuntime']