...
class PluginManager:
    ...
    def default_directory():
        ...
        # Android commonly exposes shared storage at this path. Prefer it when
        # present, while retaining the normal ~/.pyapify location elsewhere.
    ...
    def register(self, plugin):
        ...
        plugin.bind(self.app); self.plugins[name] = plugin; return plugin
    ...
    def discover(self, directory=None):
        ...
        for child in sorted(root.iterdir()):
            if child.is_dir() and (child/'main.py').is_file(): loaded.append(self.load(child))
        return loaded
    ...
    def remove(self, name):
        plugin=self.plugins.pop(name); plugin.uninstall(self.app); return plugin
    def enable(self, name): return self.plugins[name].enable()
    def disable(self, name): return self.plugins[name].disable()
