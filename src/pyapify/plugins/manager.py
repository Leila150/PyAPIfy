...
from .dependencies import resolve as resolve_dependencies
...
class PluginManager:
    ...
    def register(self, plugin: Plugin):
        ...
        for dep in plugin.dependencies():
            dep_name = dep.get("name") if isinstance(dep, dict) else str(dep).split()[0]
    ...
    def discover(self, directory=None):
        root = self._directory(directory); loaded = []
        for child in sorted(root.iterdir()):
            if child.is_dir() and (child / "main.py").is_file():
                try: loaded.append(self.load(child))
                except Exception: self.runtime.logger.exception("Failed to load plugin: %s", child)
        return loaded

    def use(self, plugin): return self.register(plugin)
    def get(self, name): return self.plugins.get(name)
    def require(self, name):
        plugin = self.get(name)
        if plugin is None: raise KeyError(f"Plugin is not loaded: {name}")
        return plugin
    ...
    def install(self, source, *, name=None, overwrite=False):
        source = Path(source).expanduser().resolve(); destination_root = self._directory()
        if source.is_dir():
            ...
            with tempfile.TemporaryDirectory() as temp:
                ...
                plugin_name = name or Plugin(source_dir).name(); destination = destination_root / plugin_name
                if destination.exists():
                    if not overwrite: raise FileExistsError(f"Plugin already installed: {plugin_name}")
                    shutil.rmtree(destination)
                shutil.copytree(source_dir, destination)
        else: raise ValueError("Plugin source must be a directory or .zip archive")
        self.runtime.log("info", "Plugin installed: %s", destination); return destination

    def uninstall(self, name, *, remove_cache=True):
        plugin = self.plugins.pop(name, None)
        if plugin is not None: plugin.uninstall(self.app)
        directory = self._directory() / name
        if directory.exists(): shutil.rmtree(directory)
        if remove_cache: self.runtime.cache(name).clear()
        self.runtime.log("info", "Plugin uninstalled: %s", name); return plugin
    ...
    def reload(self, name):
        ...
        result = self.load(directory); self.runtime.log("info", "Plugin reloaded: %s", name); return result

    def update(self, name, source=None, *, overwrite=True):
        if source is None: raise ValueError("update() requires a local plugin source")
        self.uninstall(name); destination = self.install(source, name=name, overwrite=overwrite)
        result = self.load(destination); self.runtime.log("info", "Plugin updated: %s", name); return result

    def enable(self, name):
        result = self.require(name).enable(); self.runtime.log("info", "Plugin enabled: %s", name); return result
    def disable(self, name):
        result = self.require(name).disable(); self.runtime.log("info", "Plugin disabled: %s", name); return result
    ...
    async def startup(self):
        for name in self.resolve():
            ...
            try:
                ...
                self.runtime.logger.exception("Plugin startup failed: %s", name); raise

    async def shutdown(self):
        for name in reversed(self.resolve()):
            plugin = self.plugins[name]
            try:
                ...
                self.runtime.logger.exception("Plugin shutdown failed: %s", name); raise
        self.runtime.close()
