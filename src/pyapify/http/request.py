...
class Request:
    def __init__(self, method='GET', target='/', headers=None, body=b'', client=None, scheme='http'):
        ...
        logger.debug("Request created: %s %s (%d bytes)", self.method, self.path, len(self.body))
    @property
    ...
    def json(self):
        if self._json is None:
            logger.debug("Parsing JSON request body: %s", self.path)
            self._json=json.loads(self.body.decode('utf-8') or 'null')
        return self._json
    @property
    def form(self):
        if self._form is None:
            logger.debug("Parsing form request body: %s", self.path)
    ...
    def _parse_multipart(self):
        logger.debug("Parsing multipart request: %s", self.path)
        ...
        for part in self.body.split(b'--'+boundary)[1:]:
            ...
            if not nm: continue
            name=nm.group(1)
            if fm: self._files[name]=UploadFile(name,fm.group(1),ph.get('Content-Type'),data,ph)
            else: self._form[name]=data.decode('utf-8','replace')
    @property
    def cookies(self):
        if self._cookies is None:
            logger.debug("Parsing cookies: %s", self.path)
