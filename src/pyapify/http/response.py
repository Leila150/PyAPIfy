...
class HTTPResponse(Exception):
    def __init__(self, data=None, status=200, *, detail=None, headers=None, content_type=None):
        ...
        self.data=data; self.status=validate_status(status); self.detail=detail
...
class HTTP:
    OK=Status.OK; CREATED=Status.CREATED; ACCEPTED=Status.ACCEPTED; NO_CONTENT=Status.NO_CONTENT
    BAD_REQUEST=Status.BAD_REQUEST; UNAUTHORIZED=Status.UNAUTHORIZED; FORBIDDEN=Status.FORBIDDEN; NOT_FOUND=Status.NOT_FOUND
    METHOD_NOT_ALLOWED=Status.METHOD_NOT_ALLOWED; UNPROCESSABLE_CONTENT=Status.UNPROCESSABLE_CONTENT; TOO_MANY_REQUESTS=Status.TOO_MANY_REQUESTS
    INTERNAL_SERVER_ERROR=Status.INTERNAL_SERVER_ERROR; NOT_IMPLEMENTED=Status.NOT_IMPLEMENTED; SERVICE_UNAVAILABLE=Status.SERVICE_UNAVAILABLE
    @staticmethod
    ...
    def file(path, *, status=200, filename=None, headers=None):
        p=Path(path); h=dict(headers or {}); h.setdefault('Content-Disposition',f'attachment; filename="{filename or p.name}"')
        return HTTPResponse(p.read_bytes(),status,headers=h,content_type='application/octet-stream')
    @staticmethod
    def redirect(url, *, status=302, headers=None):
        h=dict(headers or {}); h['Location']=url; return HTTPResponse(None,status,headers=h)
    @staticmethod
