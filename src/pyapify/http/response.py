...
class HTTPResponse(Exception):
    ...
    def _body_iter(self):
        ...
        if isinstance(data,(dict,list,tuple,set)):return iter((json.dumps(data,ensure_ascii=False,separators=(',',':'),default=str).encode('utf-8'),))
    ...
    def _headers(self,body_length=None):
        ...
        elif not any(k.lower()=='content-type' for k in headers) and self.data is not None:headers['Content-Type']='application/json; charset=utf-8' if not isinstance(self.data,(str,bytes)) else ('text/plain; charset=utf-8' if isinstance(self.data,str) else 'application/octet-stream')
...
def _shortcut(status):
    def make(data=None,*,detail=None,headers=None):return HTTPResponse(data,status,detail=detail,headers=headers)
    return staticmethod(make)
HTTP.ok=_shortcut(HTTP.OK);HTTP.created=_shortcut(HTTP.CREATED);HTTP.accepted=_shortcut(HTTP.ACCEPTED);HTTP.no_content=staticmethod(lambda *,headers=None:HTTPResponse(None,HTTP.NO_CONTENT,headers=headers));HTTP.bad_request=_shortcut(HTTP.BAD_REQUEST);HTTP.unauthorized=_shortcut(HTTP.UNAUTHORIZED);HTTP.forbidden=_shortcut(HTTP.FORBIDDEN);HTTP.not_found=_shortcut(HTTP.NOT_FOUND);HTTP.method_not_allowed=_shortcut(HTTP.METHOD_NOT_ALLOWED);HTTP.unprocessable=_shortcut(HTTP.UNPROCESSABLE_CONTENT);HTTP.too_many_requests=_shortcut(HTTP.TOO_MANY_REQUESTS);HTTP.server_error=_shortcut(HTTP.INTERNAL_SERVER_ERROR)
