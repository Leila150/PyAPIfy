...
def main(argv=None):
    ...
    if a.command=='routes':
    ...
    elif a.command=='benchmark': print('Use an external load generator against `pyapify run app.py` for transport benchmarks, or `pyapify test app.py` for deterministic in-process tests.')
        ...
        if a.kind=='openapi':
            ...
            target=a.output or 'pyapify_client.py';pathlib.Path(target).write_text(_client_stub(app.openapi()),encoding='utf-8');print(target)
    elif a.command=='config': print(json.dumps({'title':app.title,'version':app.version,'debug':app.debug,'max_body_size':app.max_body_size,'routes':len(app.router.routes)},indent=2))
...
def create_project(name):
    root=pathlib.Path(name);root.mkdir(parents=True,exist_ok=True);(root/'app.py').write_text('from pyapify import PyAPIfy\n\napi = PyAPIfy()\n\n@api.get("/")\ndef home():\n    return {"message": "Hello from PyAPIfy!"}\n\nif __name__ == "__main__":\n    api.run(debug=True)\n',encoding='utf-8');(root/'README.md').write_text(f'# {name}\n\nRun `pyapify dev app.py`.\n',encoding='utf-8');print(f'Created {root}/');return 0

def create_certificates(output,hostname):
    root=pathlib.Path(output);root.mkdir(parents=True,exist_ok=True);cert=root/f'{hostname}.crt';key=root/f'{hostname}.key'
    try: subprocess.run(['openssl','req','-x509','-newkey','rsa:2048','-nodes','-keyout',str(key),'-out',str(cert),'-days','365','-subj',f'/CN={hostname}'],check=True,stdout=subprocess.DEVNULL,stderr=subprocess.PIPE)
    except (FileNotFoundError,subprocess.CalledProcessError) as exc:
        print('OpenSSL certificate generation failed; install OpenSSL or provide your own certfile/keyfile.',file=sys.stderr);return 1
    print(f'Certificate: {cert}\nKey: {key}');return 0

def _client_stub(schema):
    return '# Generated by PyAPIfy\n# OpenAPI operations:\n'+json.dumps([f'{m.upper()} {p}' for p,item in schema.get('paths',{}).items() for m in item],indent=2)+'\n'
